...
let proxyServer = http.createServer((req, res) => {
    ...
    if (req.headers['x-destionation-url']){
        url = 'http://' + req.headers['x-destionation-url']
    } 
    let options = { 
        url: url + req.url
    }
    logStream.write('options:' + options.url + '\n')
    req.pipe(request(options)).pipe(res)
})
proxyServer.listen(9000)

console.log('proxyServer listening @127.0.0.1:9000')
